...
export function AppRouter() {
  return <Routes><Route path="/" element={<Navigate to={routePaths.login} replace />} /><Route path={routePaths.login} element={<LoginPage />} /><Route path={routePaths.dashboard} element={<DashboardPage />} />{pendingRoutes.map(([path, title]) => <Route key={path} path={path} element={<ComingSoonPage title={title} />} />)}<Route path="*" element={<NotFoundPage />} /></Routes>;
}
